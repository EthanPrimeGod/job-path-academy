...
function getFilteredJobs() {
  let filtered = jobs.filter(job => {
    const matchesSearch = state.searchTerm === '' ||
      ...
      job.tags.some(tag => tag.toLowerCase().includes(state.searchTerm.toLowerCase()));

    const matchesCategory = state.category === 'all' || job.category === state.category;
    const matchesExperience = state.experienceLevel === 'all' || job.experienceLevel === state.experienceLevel;
    const matchesJobType = state.jobType === 'all' || job.type === state.jobType;
    const matchesLocation = state.location === 'all' || job.location.includes(state.location);
  ...
  if (state.sortBy === 'newest') {
    filtered.sort((a, b) => parseDateString(a.postedDate) - parseDateString(b.postedDate));
  } else if (state.sortBy === 'oldest') {
    filtered.sort((a, b) => parseDateString(b.postedDate) - parseDateString(a.postedDate));
  }

  return filtered;
}

// Parse date strings like "2 days ago"
...
function renderJobs() {
  ...
  if (filteredJobs.length > 0) {
    ...
    const end = Math.min(state.currentPage * state.itemsPerPage, filteredJobs.length);
    jobCount.textContent = `Showing ${start}-${end} of ${filteredJobs.length} jobs`;
    jobCount.style.display = 'block';
  } else {
  ...
  if (paginatedJobs.length > 0) {
    jobGrid.innerHTML = paginatedJobs.map((job, index) => createJobCard(job, index)).join('');
    jobGrid.style.display = 'grid';
    emptyState.style.display = 'none';

    // Add click handlers
    document.querySelectorAll('.job-card').forEach((card, index) => {
      card.addEventListener('click', () => openModal(paginatedJobs[index]));
    });
  } else {
  ...
  return `
    <div class="job-card" style="animation-delay: ${index * 100}ms">
      ...
      <div class="job-card-header">
        <div class="job-icon hexagon-clip">
      ...
      <div class="job-card-tags">
        ...
        `).join('')}
        ${remainingTags > 0 ? `
          <span class="job-tag job-tag-more">+${remainingTags} more</span>
        ` : ''}
      </div>
      ...
      <button class="btn btn-outline btn-full job-card-btn">View Details</button>
    </div>
  `;
}
...
function renderPagination(totalPages) {
  ...
  for (let i = 1; i <= totalPages; i++) {
    html += `
      <button class="pagination-page ${state.currentPage === i ? 'active' : ''}"
...
function renderActiveFilters() {
  ...
  if (state.experienceLevel !== 'all') {
    html += `
      <span class="filter-tag">
        ${state.experienceLevel}
        <button onclick="removeFilter('experience')">
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <line x1="18" y1="6" x2="6" y2="18"></line>
            <line x1="6" y1="6" x2="18" y2="18"></line>
          </svg>
  ...
  modalBody.innerHTML = `
    <div class="modal-header-section">
      ...
      <div class="modal-header-text">
        ...
        ${job.featured ? '<span class="job-featured-badge" style="display: inline-block; margin-top: 0.5rem;">⭐ Featured</span>' : ''}
      </div>
    </div>
    ...
    <div class="modal-section">
      ...
      <p class="modal-section-text">${job.fullDescription}</p>
    </div>
    ...
    <div class="modal-actions">
      <a href="${job.applyUrl}" target="_blank" rel="noopener noreferrer" class="btn btn-hero btn-lg btn-full">
