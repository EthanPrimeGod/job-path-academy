...
export const JobCard = ({ job, onClick, index }: JobCardProps) => {
  return (
    ...
    >
      {job.featured && (
        <div className="absolute top-4 right-4 bg-primary/10 text-primary border border-primary/30 px-3 py-1 rounded-full text-xs font-medium">
          ⭐ Featured
        </div>
      )}
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {job.tags.slice(0, 3).map((tag, i) => (
          <span
            key={i}
            className="px-3 py-1 rounded-full text-xs font-medium bg-muted/50 text-muted-foreground border border-border/50 group-hover:border-primary/30 group-hover:text-primary transition-colors"
          >
            {tag}
          </span>
        ))}
